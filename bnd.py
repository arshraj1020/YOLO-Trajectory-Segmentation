...
def main():
    ...
    if not cap.isOpened():
        print(f"[CRITICAL ERROR] Could not open camera source at index {ACTIVE_CAMERA_INDEX}.", flush=True)
        return
    ...
    # Actual working dimensional context
    ret, initial_frame = cap.read()
    if not ret:
        print("[CRITICAL ERROR] Camera stream started but returned an empty frame context.", flush=True)
        cap.release()
        return
    h, w = initial_frame.shape[:2]

    state = fresh_state()
    print("\n>>> PIPELINE RUNNING. Focus the window and interact using keyboard.", flush=True)

    try:
        while True:
            ret, frame = cap.read()
            if not ret:
                print("[Pipeline] Hardware frame grab failure or stream ended.", flush=True)
                break

            state["frame_number"] += 1
            current_fid = state["frame_number"]

            # Initialize Video Writer lazily per trajectory
            if SAVE_ANNOTATED_VIDEO and state["video_writer"] is None:
                fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                state["video_writer"] = cv2.VideoWriter(output_video_path, fourcc, CAMERA_FPS, (w, h))

            # Inferencing orchestration
            detections = []
            if current_fid % YOLO_EVERY_N_FRAMES == 0:
                results = model.predict(frame, conf=CONF_THRESHOLD, imgsz=YOLO_IMGSZ, verbose=False)
                for box in results[0].boxes:
                    cls_id = int(box.cls[0])
                    class_name = model.names[cls_id]
                    if class_name == TARGET_CLASS:
                        bbox = box.xyxy[0].cpu().numpy()
                        bbox_clipped = clip_bbox(bbox, w, h)
                        anchor = upper_anchor(bbox_clipped)
                        detections.append({
                            "bbox": bbox_clipped,
                            "anchor": anchor,
            ...
            if chosen is not None:
                ...
                raw_pt = state["last_anchor"]  # Maintain lock position
            ...
            # Dynamic filtering and telemetry updates
            if raw_pt is not None:
                ...
                # Motion threshold initialization evaluator
                ...
                # Compile snapshot state
                metrics = state["last_metrics"] if state["last_metrics"] else {"bbox_width":"", "bbox_height":"", "bbox_area":"", "aspect_ratio":""}
                ...
                # Append fallback framework trace record if lost
                state["records"].append({
                    "frame_number": current_fid,
                    "tracking_source": source_str,
                    "detected": False,
            ...
            # Segment computations for live rendering assets
            current_boundaries = detect_boundaries(state["smooth_points"], state["frame_numbers"], state["start_index"])

            # Video Frame Overlay Render Strategy
            annotated_frame = frame.copy()
            start_pt_val = state["smooth_points"][state["start_index"]] if state["start_index"] is not None else None
            draw_overlay(annotated_frame, state["smooth_points"], current_boundaries, start_pt_val, state["start_frame"])
            ...
            if SHOW_WINDOWS:
                ...
                if SHOW_LIVE_GRAPH_WINDOW and (current_fid % LIVE_GRAPH_UPDATE_EVERY_N_FRAMES == 0):
                    graph_img = render_live_graph(state["smooth_points"], current_boundaries, start_pt_val)
                    cv2.imshow("Upper-Anchor Trajectory Graph", graph_img)

            # Polling Controller Hooks
            key = cv2.waitKey(PLAYBACK_DELAY_MS) & 0xFF
            if key == ord('q'):
                print("\n[Control] Terminal termination flag captured. Finalizing data...", flush=True)
                ...
                print(f"\n[Control] Reset requested. Dropping live buffers for trajectory {CURRENT_TRAJECTORY_ID}.", flush=True)
                if state["video_writer"] is not None:
                    state["video_writer"].release()
                state = fresh_state()

    except KeyboardInterrupt:
        print("\n[Control] Interrupted via terminal command interface.", flush=True)
        save_trajectory(state, w, h)
